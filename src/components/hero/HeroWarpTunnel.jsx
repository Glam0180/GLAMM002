  ...
  useEffect(() => {
    ...
    for (let s = 0; s < SPOKE_COUNT; s++) {
      spokes.push({
        ...
        radius: TUNNEL_RADIUS * (0.08 + 0.92 * Math.pow(Math.random(), 2.2)),
        z: -Math.random() * FAR_BACK - 20,
    ...
    loader.load('/fonts/helvetiker_bold.typeface.json', (font) => {
      ...
      WORDS.forEach((word) => {
        ...
        const shapes = font.generateShapes(word.repeat(3), TEXT_SIZE)
    ...
    const warpShader = {
      uniforms: { tDiffuse: { value: null }, uCenter: { value: new THREE.Vector2(0.5, 0.5) }, uStrength: { value: 0.22 }, uAberration: { value: 0.08 } },
    ...
    function animate(){
      ...
      if(fontLoaded){
        ...
        strips.forEach((strip)=>{
          ...
          if (strip.z > CAMERA_Z + 30) {
            strip.z = -FAR_BACK - Math.random() * 100
          ...
          const expansion = 0.02 + 0.98 * Math.pow(p, 2.2)
